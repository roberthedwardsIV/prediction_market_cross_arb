...
bool approve_pair(const OrderIntent& yes, const OrderIntent& no, PortfolioManager& book, const RiskLimits& limits) {
    ...
    if(yes.venue == Kalshi) { 
        if(yes_cost > kalshi_cash) { return false; }
        else if(kalshi_exposure + yes_cost > (kalshi_cash + kalshi_exposure) * (1 - limits.venue_reserve_pct)) { return false; }
    }
    else if(yes.venue == Polymarket) {
        if(yes_cost > polymarket_cash) { return false; }
        else if(polymarket_exposure + yes_cost > (polymarket_cash + polymarket_exposure) * (1 - limits.venue_reserve_pct)) { return false; }
    }
    else if(yes.venue == Gemini) {
        if(yes_cost > gemini_cash) { return false; }
        else if(gemini_exposure + yes_cost > (gemini_cash + gemini_exposure) * (1 - limits.venue_reserve_pct)) { return false; }
    }
    ...
    if(no.venue == Kalshi) { 
        if(no_cost > kalshi_cash) { return false; }
        else if(kalshi_exposure + no_cost > (kalshi_cash + kalshi_exposure) * (1 - limits.venue_reserve_pct)) { return false; }
        else if (same_venue && yes_cost + no_cost > kalshi_cash) { return false; }
        else if (same_venue && kalshi_exposure + yes_cost + no_cost > (kalshi_cash + kalshi_exposure) * (1 - limits.venue_reserve_pct)) { return false; }

    }

    else if(no.venue == Polymarket) {
        if(no_cost > polymarket_cash) { return false; }
        else if(polymarket_exposure + no_cost > (polymarket_cash + polymarket_exposure) * (1 - limits.venue_reserve_pct)) { return false; }
        else if (same_venue && yes_cost + no_cost > polymarket_cash) { return false; }
        else if (same_venue && polymarket_exposure + yes_cost + no_cost > (polymarket_cash + polymarket_exposure) * (1 - limits.venue_reserve_pct)) { return false; }
    }
    else if(no.venue == Gemini) {
        if(no_cost > gemini_cash) { return false; }
        else if(gemini_exposure + no_cost > (gemini_cash + gemini_exposure) * (1 - limits.venue_reserve_pct)) { return false; }
        else if (same_venue && yes_cost + no_cost > gemini_cash) { return false; }
        else if (same_venue && gemini_exposure + yes_cost + no_cost > (gemini_cash + gemini_exposure) * (1 - limits.venue_reserve_pct)) { return false; }
    ...
    if(total_cash == 0) { return false; }
    if(((total_exposure + proposed_spend) / total_cash) > limits.max_allocation_pct) { return false; }
    ...
    if(((market_specific_exposure + proposed_spend) / total_equity) > limits.max_market_pct) { return false; }
