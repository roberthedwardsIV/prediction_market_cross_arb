...
| `gem_id` | Gemini `instrumentSymbol`. Put `0` if this row has no Gemini leg. |

Max **16** rows. Extra lines are ignored. Two rows with the same `market_id`: the second is skipped. Two rows with the same `kal_id`, `pm_id`, or `gem_id`: quotes collide.

If a row’s `expiration` is reached **during** the session, restarting not required. That market is dropped from trading immediately (`expiration <= now`). Quotes may still print; no new orders are sent. Delete the row the next morning so it is gone from the file too.

### Where the ids come from

- **Kalshi:** ticker on the market page (and in the URL).
